...
class _SeasonsPageState extends State<SeasonsPage> with SingleTickerProviderStateMixin {
  ...
  String _selectedAprioriDataset = 'crime_data'; // Default selected dataset
  final List<String> _aprioriDatasets = ['crime_data', 'safety_data'];
  final Map<String, String> _aprioriDatasetDisplayNames = {
    'crime_data': 'Main Crime Data',
    'safety_data': 'Crime Safety Data'
  };
  ...
  Future<void> _runAprioriAnalysis() async {
    ...
    try {
      // Changed to POST request with a body
      final response = await http.post(
        Uri.parse('http://127.0.0.1:8000/api/weather_analysis'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({'dataset_name': _selectedAprioriDataset}),
      );
  ...
  Widget _buildTablesContent(Map<String, dynamic> exploratoryData) {
    ...
    return ListView(
      ...
      children: [
        ...
        _buildSeasonCategoryStatsTable(seasonCrime, 'crime_type', 'Crime Stats by Season (Main Data)'),
        const SizedBox(height: 24),
        _buildSeasonCategoryStatsTable(seasonWeapon, 'weapon_used', 'Weapon Usage Stats by Season (Main Data)'),
        const SizedBox(height: 24),
        const Divider(),
        const SizedBox(height: 24),
        Center(child: Text('Apriori Analysis Dataset', style: Theme.of(context).textTheme.titleMedium)),
        const SizedBox(height: 8),
        Center(
          child: DropdownButton<String>(
            value: _selectedAprioriDataset,
            onChanged: (String? newValue) {
              if (newValue != null) {
                setState(() {
                  _selectedAprioriDataset = newValue;
                });
              }
            },
            items: _aprioriDatasets.map<DropdownMenuItem<String>>((String value) {
              return DropdownMenuItem<String>(
                value: value,
                child: Text(_aprioriDatasetDisplayNames[value] ?? value),
              );
            }).toList(),
          ),
        ),
  ...
  Widget _buildGraphsContent(Map<String, dynamic> exploratoryData) {
    ...
    return ListView(
      ...
      children: [
        ...
        buildCrimeBarChart(context, seasonCrime, categoryKey: 'crime_type'), // Added context
        const SizedBox(height: 16),
        Center(child:Text('Weapon Usage by Season', style: const TextStyle(fontWeight: FontWeight.bold))),
        buildCrimeBarChart(context, seasonWeapon, categoryKey: 'weapon_used'), // Added context
        const SizedBox(height: 24),
        const Divider(),
        const SizedBox(height: 24),
        Center(child: Text('Apriori Analysis Dataset', style: Theme.of(context).textTheme.titleMedium)),
